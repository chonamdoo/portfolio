...
30개 중 정상 흐름은 절반뿐입니다. 나머지는 비어 있거나 어긋난 상태입니다.

| 종류 | 화면 |
|---|---|
| 빈 상태 | 검색 기록 없음, 가족 등록 전, 받은 내역 없음, 혜택 처음 |
| 상태 변형 | 혜택 홈 4종(기본·처음·쉬는 중·완료), 도착 리워드 2종(받음·오늘 완료) |
| 로딩과 오류 | 로딩 화면, 기다림 다이얼로그 2종, 에러 다이얼로그 9종 |
| 알림 | 잠금화면 알림 6종 |

빈 화면에 캐릭터와 다음 행동 버튼을 넣었습니다. "받은 내역이 없어요"에는 "갈 곳 찾으러
가기"가 붙습니다. 아무것도 없는 화면에서 사용자가 멈추지 않게 하려는 처리입니다.

### 음성 입력을 3단계로 쪼갰다

입력 중 → 말하는 중 → 말한 내용 확인. 지도 앱을 어려워하는 사용자에게는 타이핑보다 음성이
현실적이라, 인식 결과를 되읽어 확인받는 단계를 따로 뒀습니다. 잘못 들었으면 "다시 말하기"로
돌아갑니다.

### 도착 이후로 이어지는 장치

길안내가 끝나면 앱을 지웁니다. 그래서 도착하면 100원을 주고, 모아서 상품권(편의점·마트·빵집)
으로 바꾸는 흐름을 넣었습니다. 목적지 주변 장소 추천도 같은 이유입니다. 리워드는 탭 안에만
두고 지도 첫 화면에는 노출하지 않았습니다.

길을 잃었을 때 연락할 가족을 미리 등록하는 화면도 있습니다.
